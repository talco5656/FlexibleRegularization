...
iterations_so_far
    ...
    def __init__(self, model, data, **kwargs):
        ...
        self._reset()
        self._divide_to_sample_data()

    def _divide_to_sample_data(self):
        if self.eval_distribution_sample > 0:
            mask = np.random.rand(len(self.data['y_train']))
            where = np.where(mask > self.eval_distribution_sample, True, False)
            self.original_data = {
            ...
            self.sample_data = {
                'X_train': self.data['X_train'][where],  # [:args.eval_distribution_sample],
                'y_train': self.data['y_train'][where],  # [:args.eval_distribution_sample],
                'X_val': self.data['X_val'],
                'y_val': self.data['y_val'],
            }
            self.rest_of_data = {
                'X_train': self.data['X_train'][where == False],
                'y_train': self.data['y_train'][where == False],
                'X_val': self.data['X_val'],
                'y_val': self.data['y_val'],
            }
    ...
    def _step(self, t, update_var=False):
        """
        ...
        if self.logger:
            self.logger.report_scalar(value=loss, title='Classification Loss', series='solver', iteration=t)
    ...
    def meta_train_eval_distribuiton_sample(self):
        ...
        num_iteration = self.num_epochs * iterations_per_epoch
        meta_iterations = max(1, num_iteration // self.model.iter_length)
        for meta_iteration in range(meta_iterations - 1):
            print(f"Meta iteration {meta_iteration} out of {meta_iterations-1}")
            self.X_train = self.rest_of_data['X_train']
            self.y_train = self.rest_of_data['y_train']
            self.X_val = self.rest_of_data['X_val']
            self.y_val = self.rest_of_data['y_val']
            iterations_so_far = self.train(start_iteration=meta_iteration, update_var=False,
                                           meta_iter_fraction=1-self.eval_distribution_sample)
            # change to sample data
            self.X_train = self.sample_data['X_train']
            self.y_train = self.sample_data['y_train']
            self.X_val = self.sample_data['X_val']
            self.y_val = self.sample_data['y_val']
            iterations_so_far = self.train(start_iteration=meta_iteration, update_var=True,
                                           meta_iter_fraction=self.eval_distribution_sample)
            print(f"iteration so far: {iterations_so_far}")
            # if self.model.adaptive_var_reg or self.model.adaptive_dropconnect:
            self.update_param_variances()
        self.X_train = self.original_data['X_train']
        self.y_train = self.original_data['y_train']
    ...
    def train(self, start_iteration=0, update_var=False, meta_iter_fraction=None):  # iterations_so_far=0):
        ...
        meta_iterations = max(1, num_iterations // self.model.iter_length)
        meta_iterations_size = num_iterations // meta_iterations
        # num_iterations = total_iterations // meta_iterations
        end_at = (start_iteration+1)*num_iterations // meta_iterations
        if meta_iter_fraction:
            end_at = end_at*meta_iter_fraction
        for t in range(start_iteration * meta_iterations_size, end_at): # (start_iteration+1)*num_iterations // meta_iterations):
            self._step(t, update_var=update_var)
    ...
    def update_param_variances_naive(self):
        for param_name, trajectory in self.model.param_trajectories.items():
            ...
            if self.model.adaptive_dropconnect:
                dropconnect_value = 1/2 + np.sqrt(1-4*var) / 2
                dropconnect_value = np.nan_to_num(dropconnect_value, nan=0.5)
                if self.model.divide_var_by_mean_var:
