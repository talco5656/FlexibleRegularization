...
def parse_args():
    ...
    parser.add_argument("--model", default='cnn', choices=['mlp', 'cnn'])
    parser.add_argument("--num_trains", default=10000, type=int)
    parser.add_argument("--num_of_repeats", default=50, type=int)
    parser.add_argument("--dropconnect", default=0.9, type=float)
    parser.add_argument("--adaptive_var_reg", default=0, type=int)
    parser.add_argument("--reg_strength", default=None, type=int)
    parser.add_argument("--adaptive_dropconnect", default=1, type=int)
    ...
    if args.adaptive_var_reg or args.adaptive_dropconnect or args.dropconnect != 1:
        if args.model == 'mlp':
            model = FullyConnectedNet([args.fc_width] * 5, iter_length=iter_length, weight_scale=5e-2,
                                      reg=1 if args.divide_var_by_mean_var else reg_strenght, adaptive_reg=args.adaptive_var_reg,
                                      divide_var_by_mean_var=args.divide_var_by_mean_var,
                                      dropconnect=dropconnect, adaptive_dropconnect=args.adaptive_dropconnect,
                                      var_normalizer=1)

        elif args.model == 'cnn':
        # if args.adaptive_var_reg or args.addaptive_dropconnect or arg.dropconnect != 1:
            model = ThreeLayerConvNet(weight_scale=0.001, hidden_dim=args.fc_width, reg=reg_strenght, iter_length=iter_length,
                                      adaptive_dropconnect=args.adaptive_dropconnect, dropconnect=dropconnect, var_normalizer=var_normalizer)
        solver = AdaptiveSolver(model, data, print_every=args.print_every,
                                num_epochs=args.epochs, batch_size=batch_size, #.batch_size,
...
#     original_params = {}
    ...
    learning_rate = 1e-3 #trial.suggest_loguniform('learning_rate', 1e-5, 1e-3)
    reg_strenght = 0 #trial.suggest_loguniform('reg_strength', 1e-15, 1)
    var_normalizer = trial.suggest_loguniform('var_normalizer', 0.1, 10)
    dropconnect = trial.suggest_uniform('dropconnect', 0.7, 1)
    # reg_strenght = trial.suggest_loguniform('reg_strength', 1e-15, 1)
    batch_size = 100 #trial.suggest_int('bath_size', 50, 125, 25)
    ...
    model, solver, train_action = get_model(args, data=small_data, reg_strenght=reg_strenght,
                                            learning_rate=learning_rate,
                                            batch_size=batch_size, iter_length=iter_length, dropconnect=dropconnect,
                                            var_normalizer=var_normalizer)  #[args.optimizer])
