...
sys.path.extend(['/home/galh/Projects/un/CS231nAssignment/assignment2'])
...
ITER_LENGTH = 500
# def rel_error(x, y):
#   """ returns relative error ""/usr/bin/python3.6 /opt/pycharm-2018.2.4/helpers/pydev/pydevconsole.py 35843 44697
...
#   model = FullyConnectedNetOriginal([H1, H2], input_dim=D, num_classes=C,
...
#     print('%s relative error: %.2e' % (name, rel_error(grad_num, grads[name])))
#
#   print('adaptive reg - Running check with reg = ', reg)
...
# #                     )
#
#             # AdaptiveSolver(model, small_data,
#             #         print_every=10, num_epochs=20, batch_size=25,
#             #         update_rule='sgd', verbose=False,
#             #         optim_config={
#             #             'learning_rate': learning_rate,
...
#         model = FullyConnectedNetOriginal([100, 100],
#                       weight_scale=weight_scale, dtype=np.float64)
#         solver = Solver(model, small_data,
#                print_every=50, num_epochs=20, batch_size=25,
#                update_rule='sgd', verbose=False,
#                optim_config={
#                    'learning_rate': learning_rate,
#                }
...
#
#
# # def first_experiment(weight_scales, solvers):
...
# for learning_rate in learning_rates:
#     for weight_scale in weight_scales:
#         model = FullyConnectedNet([100, 100], iter_length=ITER_LENGTH,
#                       weight_scale=weight_scale, dtype=np.float64)
...
# #             model = FullyConnectedNet([100, 100],
# #                           weight_scale=weight_scale, dtype=np.float64)
#         adaptive_solver.meta_train()  ##meta_train()
#         print("weight_scale %f learning_rate %f "%(weight_scale, learning_rate))
#         print(adaptive_solver.train_acc_history)
#         print(adaptive_solver.val_acc_history)
#
#         plt.plot(adaptive_solver.loss_history, 'o')
#         plt.title('Adaptive solver training loss history')
#         plt.xlabel('Iteration')
#         plt.ylabel('Training loss')
#         plt.show()
#
# # first_experiment(weight_scales, solvers)
...
#     for learning_rate in learning_rates:
#         for weight_scale in weight_scales:
#             model = FullyConnectedNetOriginal([WIDTH]*4,
#                             weight_scale=weight_scale, dtype=np.float64)
#             solver = Solver(model, small_data,
#                             print_every=50, num_epochs=20, batch_size=25,
#                             update_rule='sgd',verbose=False,
...
#             print(solver.val_acc_history)
#
#             model = FullyConnectedNet([WIDTH]*4, iter_length=ITER_LENGTH,
#                             weight_scale=weight_scale, dtype=np.float64)
#             adaptive_solver = AdaptiveSolver(model, small_data,
#                             print_every=50, num_epochs=20, batch_size=25,
#                             update_rule='sgd',verbose=False,
...
#
#     plt.plot(solver.loss_history, 'o')
#     plt.plot(adaptive_solver.loss_history, '-')
#     plt.title('Training loss history')
#     plt.xlabel('Iteration')
#     plt.ylabel('Training loss')
#     plt.show()
...
for num_train in num_trains:
    ...
    for reg_strenght in reg_strenghts:
        ...
        adaptive_solvers = {}
            ...
            for update_rule in ['sgd', 'sgd_momentum', 'adam', 'rmsprop']:
                ...
                model = FullyConnectedNet([WIDTH]*5, iter_length=ITER_LENGTH, weight_scale=5e-2) #, reg=reg_strenght)

                solver = Solver(model, small_data, print_every=500,
                ...
                # for update_rule in ['sgd', 'sgd_momentum', 'adam', 'rmsprop']:
                print('running adaptive with ', update_rule)
                model = FullyConnectedNet([WIDTH]*5, iter_length=ITER_LENGTH, weight_scale=5e-2) #, reg=reg_strenght)

                adaptive_solver = AdaptiveSolver(model, small_data, print_every=500,
                                  num_epochs=EPOCHES, batch_size=100,
                                  update_rule=update_rule,
                                  optim_config={
                                    'learning_rate': learning_rates[update_rule],
    ...
    print(best_adaptive_descriotion)
        ...
        #     print('running with ', update_rule)
        #     model = FullyConnectedNetOriginal([WIDTH]*5, weight_scale=5e-2)
        #
        #     solver = Solver(model, small_data,
        #                     num_epochs=EPOCHES, batch_size=100,
        #                     print_every=500,
        ...
        #
        #     print('running adaptive with ', update_rule)
        #     model = FullyConnectedNet([WIDTH]*5, iter_length=ITER_LENGTH, weight_scale=5e-2)
        #
        #     adaptive_solver = AdaptiveSolver(model, small_data,
        #                                      print_every=500,
        #                                      num_epochs=EPOCHES, batch_size=100,
        ...
        #     plt.subplot(3, 1, 1)
        #     plt.plot(representation(solver.loss_history), 'o', label=update_rule)
        #
        #     plt.subplot(3, 1, 2)
        ...
        #
        #
        # for update_rule, solver in list(adaptive_solvers.items()):
        #     plt.subplot(3, 1, 1)
        #     plt.plot(representation(solver.loss_history), 'o', label=f"adaptive {update_rule}")
        #
        #     plt.subplot(3, 1, 2)
        #     plt.plot(solver.train_acc_history, '-o', label=f"adaptive {update_rule}")
        #
        #     plt.subplot(3, 1, 3)
        #     plt.plot(solver.val_acc_history, '-o', label=f"adaptive {update_rule}")
        #
        # for i in [1, 2, 3]:
        #     plt.subplot(3, 1, i)
        #     plt.legend(loc='upper center', ncol=4)
...
# from cs231n.optim import adam
        ...
        #   [ 0.69966,     0.68908best_model = None
################################################################################
...
#         model = FullyConnectedNetOriginal([600, 500, 400, 300, 200, 100], dtype=np.float64,reg=1e-2, weight_scale=weight_scale)
#
...
# print('Validation set accuracy: ', (y_val_pred == data['y_val']).avg())
# print('Test set accuracy: ', (y_test_pred == data['y_test']).avg())
#
# y_test_pred = np.argmax(best_adaptive_model.loss(data['X_test']), axis=1)
# y_val_pred = np.argmax(best_adaptive_model.loss(data['X_val']), axis=1)
# print('Validation set accuracy: ', (y_val_pred == data['y_val']).avg())
# print('Test set accuracy: ', (y_test_pred == data['y_test']).avg())382,  0.67851319,  0.66794809,  0.65738853,],
