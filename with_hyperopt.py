...
def get_model(args, reg_strenght=0.1):
    if args.model == 'mlp':
...
def train_and_eval_single_model(args, learning_rate):
...
#     original_params = {}
    ...
    # batch_size = trial.suggest_int('bath_size', 50, 100, 25)
    model = get_model(args)
    # reg_strenghts = [1, 0.5, 0.1, 1e-2, 5e-3, 0]
    if args.adaptive_var_reg or args.adaptive_dropconnect or args.dropconnect != 1:
        solver = AdaptiveSolver(model, small_data, print_every=args.print_every,
                                     num_epochs=args.epochs, batch_size=50, #trial['batch_size'],
                                     update_rule=optimizer,
                                     optim_config={
                                         'learning_rate': triallearning_rate,
                                     },
...
def main(args):
    ...
    def objective(space):
        func = functools.partial(train_and_eval_single_model, args)
    ...
    space = {'learning_rate':
    hp.uniform('learning_rate', 1e-5, 1e-3)
             # 'reg_strenght':
    # hp.loguniform('reg_strenght', 0, 1),
    #          'batch_size':
    # hp.uniform('batch_size', range(50, 125, 25)),
             }
    trials = Trials()
    best = fmin(objective,
        space=space,   #hp.uniform('learning_rate', 1e-5, 1e-3),
        algo=tpe.suggest,
        max_evals=2,
        trials=trials)
...
def _main(args):
    ...
    # search_space = {
    # 'learning_rate':
    # tune.uniform(0.001, 0.01),
    #          'reg_strenght':
    # tune.uniform(1e-15, 1),
    # # }
    # #          'batch_size':
    # # tune.uniform(50, 125),
    # }
    ...
    # study.optimize(analysis, n_trials=args.num_of_repeats)
